...
function eventInterceptor(routerInstance) {
  for (let k in routerInstance.options.routes) {
    //遍历路由实例，获取所有单页面组件
...
function outerRecursion(ele) {
  if (typeof ele === "function") {
    ...
    if (ele.name == "component" && String(ele).includes("AMD")) {
      //amd方式加载 component:resolve => require(['@/pages/Lottery/Wheel'], resolve)
      ele(e => innerRecursion(e.default));
    } else {
      ele().then(e => {
